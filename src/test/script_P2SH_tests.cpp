...
BOOST_AUTO_TEST_CASE(norecurse)
{
    ...
    invalidAsScript << OP_INVALIDOPLODE << OP_INVALIDOPLODE;
    ...
    // Should not verify, because it will try to execute OP_INVALIDOPLODE
    BOOST_CHECK(!Verify(scriptSig, p2sh, true, err));
    BOOST_CHECK_MESSAGE(err == SCRIPT_ERR_BAD_OPLODE, ScriptErrorString(err));
...
BOOST_AUTO_TEST_CASE(is)
{
    ...
    // Not considered pay-to-script-hash if using one of the OP_PUSHDATA oplodes:
