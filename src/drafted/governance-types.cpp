...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "OplNetwork") type = OplNetwork;
    if(strType == "OplNetworkVariable") type = OplNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == OplNetwork) s = "OplNetwork";
    if(type == OplNetworkVariable) s = "OplNetworkVariable";
