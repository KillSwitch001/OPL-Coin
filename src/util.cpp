...
const char * const BITCOIN_CONF_FILENAME = "opl.conf";
const char * const BITCOIN_PID_FILENAME = "opld.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "opl" is a composite category enabling all Opl-related debug output
            if(ptrCategory->count(string("opl"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OplCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OplCore
    // Mac: ~/Library/Application Support/OplCore
    // Unix: ~/.oplcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OplCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/OplCore";
#else
    // Unix
    return pathRet / ".oplcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty opl.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override opl.conf
