...
*Setup instructions for a Gitian build of Opl Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Opl
...
- [Building Opl Core](#building-opl-core)
...
Clone the git repositories for Opl Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/opl-project/opl.coin
```
...
in the Opl Core repository under 'Fetch and build inputs' to install sources which require
...
To build Opl Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Opl Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/opl/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/opl-project/opl.coin
...
For example:
```bash
URL=https://github.com/crowning-/opl.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit opl=${COMMIT} --url opl=${URL} ../opl/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit opl=${COMMIT} --url opl=${URL} ../opl/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit opl=${COMMIT} --url opl=${URL} ../opl/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the opl git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../opl/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/opl-project/opl.coin-detached-sigs.git

BTCPATH=/some/root/path/opl.git
SIGPATH=/some/root/path/opl-detached-sigs.git

./bin/gbuild --url opl=${BTCPATH},signature=${SIGPATH} ../opl/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/opl-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/opl-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/opl-osx-build.assert
```
...
[opl/gitian.sigs](https://github.com/oplbanana/gitian.sigs/) repository, or if that's not possible to create a pull
request.
